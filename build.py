#!/usr/bin/env python3
"""Build obfuscated index.html from data.json + dynamics.json → _deploy/"""
import json, os, re, sys

ROOT = os.path.dirname(os.path.abspath(__file__))
DEPLOY = os.path.join(ROOT, '_deploy')

# ── Class name map (short = obfuscated) ──
C = {
    'header':'a','logo':'b','nav':'c','hero':'d',
    'card':'e','stats':'f','stat':'g','tl-wrap':'h',
    'tl-col':'i','tl-list':'j','tl-node':'k','tl-date':'l',
    'tl-box':'m','tl-empty':'n','footer':'o',
    'mob-tl':'p','mob-tl-item':'q','mob-tl-date':'r',
    'mob-tl-box':'s','mob-tl-empty':'t','fans-bar':'u',
    'bottom-nav':'v','section':'w','search':'x',
    'icon':'y','title':'z','subtitle':'A','btn':'B',
    'desktop-layout':'C','mobile-layout':'D',
    'stat-label':'E',
}
...
def dyn_html(items, mob):
    ...
    nc, dc, bc = cl('mob-tl-item') if mob else cl('tl-node'), cl('mob-tl-date') if mob else cl('tl-date'), cl('mob-tl-box') if mob else cl('tl-box')
    ps = []
    for it in items:
        t = it.get('content', '')
        v = it.get('video', {})
...
def build():
    ...
    css = '\n'.join([
        '*{margin:0;padding:0;box-sizing:border-box}body{font-family:"Microsoft YaHei",sans-serif;background:#050816;color:#fff}',
        ...
        f'.{cl("C")} .{cl("c")}{{display:flex;gap:10px;flex-wrap:wrap}}.{cl("C")} .{cl("c")} a{{color:white;text-decoration:none;padding:8px 14px;border-radius:8px;background:0 0;font-size:14px}}.{cl("C")} .{cl("c")} a:hover{{background:#00d9ff22}}',
        f'.{cl("C")} .{cl("x")} input{{padding:10px 15px;border:none;border-radius:8px;background:#111b3a;color:white;width:240px}}',
        ...
        f'.{cl("D")} .{cl("v")}{{position:fixed;bottom:0;left:0;width:100%;height:65px;background:#08101f;border-top:1px solid rgba(0,229,255,.3);display:flex;justify-content:space-around;align-items:center}}.{cl("D")} .{cl("v")} a{{color:white;text-decoration:none;font-size:13px;text-align:center;background:0 0}}.{cl("D")} .{cl("v")} span{{display:block;font-size:22px}}',
        '.sb{display:flex;gap:8px;margin-bottom:12px}.sb input{flex:1;padding:10px 14px;border:none;border-radius:8px;background:#111b3a;color:white;font-size:14px}.sb input:focus{outline:2px solid #0df}.sb button{background:#333;border:none;color:#999;border-radius:8px;padding:0 14px;cursor:pointer}.scnt{font-size:12px;color:#666;margin:-8px 0 10px}'
    ])
    css = re.sub(r'\s+', ' ', css)
    ...
    html = f'''<!DOCTYPE html><html lang="zh-CN"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>My World</title><style>{css}</style></head><body><div class="C"><a class="a"><div class="b">My World</div><nav class="c"><a href="#">\u9996\u9875</a><a href="#">\u4ecb\u7ecd</a><a href="#">\u76f4\u64ad</a><a href="https://space.bilibili.com/394065596" target="_blank">\u96ea\u68a8B\u7ad9</a></nav><div class="x"><input type="text" placeholder="\u641c\u7d22\u52a8\u6001" id="q" oninput="F()"></div></a><section class="d"><div class="e"><h1>Hello, I am Myself</h1><p>\u4e2a\u4eba\u4e3b\u9875</p><div class="f"><div class="g"><b>0</b>\u9879\u76ee</div><div class="g"><b>0</b>\u6587\u7ae0</div><div class="g"><b>{fans}</b>\u7c89\u4e1d</div><div class="g"><b>{dcnt}</b>\u52a8\u6001</div></div></div><div class="e"><h1>{esc(name)}</h1><p>{esc(live_t)}</p><div class="f"><div class="g"><b>{live_text}</b><span class="E">\u76f4\u64ad</span></div><div class="g"><b>{fans}</b>\u7c89\u4e1d</div><div class="g"><b>{works}</b>\u4f5c\u54c1</div><div class="g"><b>{dcnt}</b>\u52a8\u6001</div></div></div></section><section class="h"><div class="i"><h2>\u52a8\u6001</h2><div class="sb"><input type="text" id="q2" placeholder="\u641c\u7d22\u52a8\u6001..." oninput="F()"><button onclick="C()">\u2715</button></div><div class="scnt" id="sc">\u5171 {cnt} \u6761</div><div class="j" id="tl">{dyn_d}</div></div></section><footer class="o">\u00a9 2026 My World \u00b7 By Mcldy</footer></div><div class="D"><a class="a"><div class="y">☰</div><div class="b">My World</div><div class="y">🔍</div></a><section class="d"><h1 class="z">Welcome To My World</h1><p class="A">Connecting Two Worlds</p></section><div class="u"><span id="nn">{esc(name)}</span> \u7c89\u4e1d <b>{fans}</b> \u00b7 \u76f4\u64ad: <b>{live_text}</b> \u00b7 \u4f5c\u54c1: <b>{works}</b> \u00b7 \u52a8\u6001: <b>{dcnt}</b></div><section class="w"><div class="e"><h3>\u52a8\u6001</h3><div class="sb"><input type="text" id="q3" placeholder="\u641c\u7d22..." oninput="F()"><button onclick="C()">\u2715</button></div><div class="scnt" id="sc2">\u5171 {cnt} \u6761</div><div class="p" id="tml">{dyn_m}</div></div></section><nav class="v"><a href="#"><span>🏠</span>\u9996\u9875</a><a href="#"><span>📜</span>\u52a8\u6001</a><a href="https://space.bilibili.com/394065596" target="_blank"><span>🌸</span>B\u7ad9</a></nav></div><script>var D={dv};function S(){{var a=document.getElementById('q'),b=document.getElementById('q2'),c=document.getElementById('q3');return(a?a.value:'')||(b?b.value:'')||(c?c.value:'')}}function F(){{var k=S().toLowerCase(),t=document.getElementById('tl'),m=document.getElementById('tml'),sc=document.getElementById('sc'),sc2=document.getElementById('sc2'),c=0;D.forEach(function(d,i){{var txt=(d.content+' '+(d.video&&d.video.title||'')+' '+d.date).toLowerCase();if(t.children[i])t.children[i].style.display=k&&!txt.includes(k)?'none':'';if(m&&m.children[i])m.children[i].style.display=k&&!txt.includes(k)?'none':'';if(!t.children[i]||t.children[i].style.display!='none')c++}});if(sc)sc.textContent='\u5171 '+c+' \u6761';if(sc2)sc2.textContent='\u5171 '+c+' \u6761'}}function C(){{var a=document.getElementById('q'),b=document.getElementById('q2'),c=document.getElementById('q3');if(a)a.value='';if(b)b.value='';if(c)c.value='';F()}}(function(){{var m=/Mobi|Android|iPhone|iP(ad|od)/i.test(navigator.userAgent)||window.innerWidth<768;document.body.className=m?'D':'C'}})();</script></body></html>'''

    html = re.sub(r'\s+', ' ', html)
    html = re.sub(r'> <', '><', html)
    html = re.sub(r'<script>(.*?)</script>', lambda m: '<script>' + m.group(1).replace(' ', '') + '</script>', html, flags=re.DOTALL)
    ...
    print(f'Built: _deploy/index.html ({size:,} bytes, {cnt} dynamics)')
    return True
